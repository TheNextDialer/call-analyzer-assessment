...
Set two environment variables, then run the tests. The exact syntax depends on your shell:
...
**Windows PowerShell**
```powershell
$env:ASSESSMENT_API = "<from the assessment app>"
$env:ASSESSMENT_TOKEN = "<from the assessment app>"
npm test
```

The two variables bind the run to your assessment session — the runner asks the server for a verification code at the end of the test output, and you copy/paste the whole thing into the submission form. If you run `npm test` without those env vars set, you'll still see test results, but the verification line will be skipped and your submission won't pass auto-grade.
